...
import { getBooleanSetting, getSetting, setSetting } from "./settingsService";

export type TelemetryAction = "UNINSTALL" | "DISABLE" | "RESTORE" | "ENABLE" | "UNDO";
...
function getTelemetrySalt(): string {
  const existing = getSetting("telemetry_salt");
  if (existing && existing.trim().length > 0) return existing;

  const generated = randomUUID();
  setSetting("telemetry_salt", generated);
  return generated;
}

function hashPackageId(packageName: string): string {
...
function bucketError(errorMessage?: string): string | null {
  if (!errorMessage) return null;

  const value = errorMessage.toLowerCase();
  if (value.includes("permission") || value.includes("denied")) return "permission";
  if (value.includes("not found") || value.includes("unknown package")) return "not_found";
  if (value.includes("timeout") || value.includes("timed out")) return "timeout";
  if (value.includes("offline") || value.includes("unauthorized") || value.includes("no devices")) return "device_connection";
  return "other";
}
...
export function recordActionOutcome(input: TelemetryActionInput): boolean {
  ...
  try {
    ...
    const deviceIdHash = createHash("sha256")
      .update(`${getTelemetrySalt()}:${input.deviceId}`)
      .digest("hex");
    ...
    stmt.run(
      ...
      input.action,
      input.success ? 1 : 0,
      bucketError(input.errorMessage),
      hashPackageId(input.packageName),
      deviceIdHash,
      input.deviceBrand || null,
      input.deviceModel || null,
      input.androidSdk ?? null,
      input.modelLabel || null,
      typeof input.modelConfidence === "number" ? input.modelConfidence : null,
...
export function getTelemetrySummary(days = 30): TelemetrySummary {
  ...
  return {
    ...
    rollback_rate: destructiveSuccess > 0 ? undoSuccess / destructiveSuccess : null,
...
export function getRetrainingSignals(days = 30): {
  ...
  return {
    enabled,
    low_confidence_rate:
      stats?.low_confidence_rate === null ? null : Number(stats.low_confidence_rate),
